	...
	global.fetch = function (url, options) {
		...
		return new Promise(function (resolve, reject) {
			...
			xhr.onerror = function () {
				// status/readyState distinguish a real network failure (0/DNS/TLS/refused)
				// from a same-origin-policy block (also fires onerror, per the XHR spec,
				// with no way to tell them apart except readyState/status both being 0 in
				// both cases - logged anyway so a future failure has more to go on).
